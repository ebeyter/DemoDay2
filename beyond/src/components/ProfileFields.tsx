...
/**
 * Profil sorularının TEK KAYNAĞI.
 *
 * Aynı sorular iki ekranda görünüyor: kayıt sihirbazı (`/profile`) ve
 * ayarlardaki "Profilim" sekmesi. Sorular kopyalanmadı; ikisi de buradaki
 * bileşenleri çağırıyor. Yeni bir alan eklendiğinde iki ekranda birden
 * çıkıyor ve "sihirbazda sordu ama ayarlarda düzenleyemiyorum" durumu
 * yapısal olarak imkânsız hale geliyor.
 *
 * Adım sırası, ilerleme çubuğu ve kaydetme burada DEĞİL: onlar iki ekranda
 * farklı (sihirbaz adım adım ilerliyor, ayarlar hepsini birden gösteriyor).
 */
 ...
 */
export const GRADUATION_YEARS = [2027, 2028, 2029, 2030, 2031];
...
export function BasicsFields({ draft, update }: ProfileFieldsProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Lise TÜRÜ değil, alınan DİPLOMA. Üniversiteler "Anadolu Lisesi"
          bilgisine göre şart koymuyor; "IB 38 puan", "A-level A*AA" diye
          koyuyor. Çoktan seçmeli, çünkü bir öğrenci aynı anda Türk lise
          diploması + AP dersleri taşıyabiliyor. */}
...
export function InterestFields({ draft, update }: ProfileFieldsProps) {
  ...
  return (
    <div>
      ...
      {/* Her alanın yanında KATALOGDAKİ PROGRAM SAYISI yazıyor. Sebep: alan
          listesi katalogdan bağımsız genişletildi (hukuk, mimarlık, tasarım…)
          ve karşılığı olmayan bir alanı seçen öğrenci boş sonuç ekranıyla
          karşılaşıyor — "formu doldurdum, eşleşme gelmedi" şikâyetinin sebebi
          tam olarak bu. Sayı koddan geliyor, katalog büyüdükçe kendiliğinden
          güncelleniyor. */}
      <div className="flex flex-wrap gap-2">
        {FIELD_IDS.map((id: FieldId) => {
          ...
          return (
            ...
            >
              ...
              {pick(FIELDS[id].name)}
              <span
                className={cx("ml-1.5 text-[11px]", count === 0 ? "text-danger" : "opacity-60")}
              >
...
export function GradeFields({ draft, update }: ProfileFieldsProps) {
  ...
  return (
    <div>
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        >
          <Input
            ...
            // Sınıf ortalamaları girildiyse bu alan onlardan TÜRETİLİYOR ve
            // elle değiştirilemiyor: iki farklı sayı tutmak, hangisinin doğru
            // olduğu belirsiz bir profil üretir. Hiç sınıf girilmediyse
            // öğrenci genel ortalamasını doğrudan yazabiliyor.
      ...
      {/* SINIF SINIF ORTALAMA — her yıl İSTEĞE BAĞLI. Öğrenci kaç yıl okuduysa
          o kadarını giriyor; 11. sınıftaki birinden 12. sınıf notu istemek
          anlamsız. Genel ortalama girilenlerin düz ortalaması (kredi
          ağırlığını bilmiyoruz, uydurmuyoruz). */}
      <Field label={t.wizard.grades.byYear} hint={t.wizard.grades.byYearHint} className="mt-5">
              ...
              <div key={year}>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    const others = (draft.gradeYears ?? []).filter((g) => g.year !== year);
                    // Boş bırakmak = "bu yılı okumadım/girmiyorum". Kaydı
                    // diziden çıkarıyoruz ki ortalamaya 0 olarak katılmasın.
                    const next =
                      raw === "" ? others : [...others, { year, average: Number(raw) }];
      ...
      {/* Çevrilmiş notu anında göstermek güven veriyor — öğrenci neyle
          karşılaştırıldığını görüyor. */}
      <div className="mt-4 p-4 rounded-xl bg-accent-soft border border-accent-line">
        <div className="flex items-baseline justify-between gap-3">
          ...
          <span className="text-[22px] font-semibold text-accent tabular-nums">
            {toHundredScale(derivedAverage ?? draft.gpa, draft.gpaScale).toFixed(0)}
...
// ---------------------------------------------------------------------------
// 4 — Dersler ve sınavlar
// ---------------------------------------------------------------------------

export function SubjectFields({ draft, update }: ProfileFieldsProps) {
  const { t, pick } = useLocale();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AP DERSLERİ TEK TEK. Önceden tek bir "AP (en yüksek notun)" puanı
          vardı; bir öğrenci AP Calculus'tan 5, AP Biology'den 3 alabiliyor ve
          tek sayı bunu temsil etmiyor. */}
          ...
          {(draft.apCourses ?? []).map((course, index) => (
            <div key={index} className="flex gap-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  update({ apCourses: (draft.apCourses ?? []).filter((_, i) => i !== index) })
                }
              >
                {t.common.remove}
              </Button>
      ...
      {/* YKS SİHİRBAZIN ÖNÜNDEN KALKTI ama silinmedi: Almanya'nın kapı şartı
          bu. `de-tum-informatics` ve `de-rwth-mechanical` kayıtlarında zorunlu
          ve COUNTRIES.DE notu da bunu söylüyor. Tamamen kaldırsak öğrenci
          Almanya'nın en kritik detayını hiç görmezdi; burada isteğe bağlı. */}
      <Field label={t.wizard.subjects.yks} hint={t.wizard.subjects.yksHint}>
        <Input
          type="number"
          inputMode="numeric"
          placeholder="—"
          value={draft.standardizedTests.find((test) => test.test === "yks")?.score ?? ""}
          onChange={(e) => {
  ...
  return (
    <div>
      ...
      {/* YKS ve AP bu listeden ÇIKARILDI, silinmedi:
          - YKS "Dersler ve sınavlar" adımına taşındı (Almanya kapı şartı
            olduğu için orada bağlamıyla duruyor)
          - AP artık ders ders giriliyor, tek "en yüksek notun" puanı bir
            öğrencinin AP profilini temsil etmiyordu
              ...
              update({
                standardizedTests:
                  score === undefined
                    ...
                    : [...draft.standardizedTests.filter((s) => s.test !== test), { test, score }],
              })
            }
          />
        ))}
...
/**
 * Dil belgesi satırı — PUAN İSTEMİYOR.
 *
 * Öğrenci "IELTS'im var" diyebiliyor, kaç aldığını yazmak zorunda değil.
 * Sebep: sihirbazın en çok terk edilen adımı buydu; sekiz sınavın puan
 * ölçeğini hatırlamak (Duolingo 10-160, Cambridge 100-230…) formu doldurmayı
 * bırakma sebebiydi. Puanı kendimiz uydurmuyoruz da: işaretlenen sınav
 * `score: 0` ile kaydediliyor ve eşleştirme motoru bunu "belge var, puan
 * bilinmiyor" olarak okuyup şartı EKSİK BİLGİ sayıyor — geçmiş de saymıyor,
 * başarısız da. Öğrenci puanını sonradan girmek isterse eylem listesinde
 * karşısına çıkıyor.
 */
function LanguageRow({
  ...
  const scale = LANGUAGE_TEST_SCALES[test];
  ...
  return (
    <label
      htmlFor={`lang-${test}`}
      className={cx(
        "flex items-center gap-3 px-3.5 py-2.5 rounded-xl border transition-colors cursor-pointer",
...
function ScoreRow({
  ...
  testId: string;
  scale: (typeof STANDARDIZED_TEST_SCALES)[StandardizedTest];
  value: number | undefined;
  onChange: (score: number | undefined) => void;
}) {
  const { pick } = useLocale();
