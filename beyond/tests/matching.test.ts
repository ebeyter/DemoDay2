...
// ---------------------------------------------------------------------------
// README regresyon senaryosu — gerçek katalogla
// ---------------------------------------------------------------------------
...
describe("README demo senaryosu — gerçek katalogla regresyon", () => {
  ...
  //
  // Sayılar `npm run check-demo` ile doğrulandı: Uygun 2→5, Zorlayıcı 15→12.
  // Kırılırsa hem bu test hem README'nin "Test edilmiş örnek" cümlesi
  // yanlış olur — ikisini birlikte güncelle.
  //
  // GÜNCELLEME (2026-08-14): Zorlayıcı sayıları 10→7 iken 15→12 oldu. Sebep
  // hata değil: katalog 36'dan 42 programa çıktı (Oxford, Edinburgh, KCL,
  // Sapienza, Torino, EDHEC) ve yeni kayıtların çoğu teyit edilemeyen alan
  // taşıdığı için `unknown` üretip `reach` bandına düşüyor.
  //
  // DEĞİŞMEYEN ŞEY, ASIL İDDİA BU: her iki bantta da fark ±3. Yani "IELTS'i
  // 6'dan 7'ye çıkarmak üç programı Zorlayıcı'dan Uygun'a taşıyor" cümlesi
  // katalog büyüdükçe de doğru kaldı. Aşağıdaki delta kontrolleri o yüzden
  // mutlak sayılardan daha değerli — katalog her değiştiğinde mutlak sayılar
  // kayar, iddia kaymaz.
  ...
  it(`IELTS ${DEMO_IELTS_BEFORE} iken Uygun 2, Zorlayıcı 15 program içerir`, () => {
    const counts = bandCounts(DEMO_IELTS_BEFORE);
    expect(counts.match).toBe(2);
    expect(counts.reach).toBe(15);
  });

  it(`IELTS ${DEMO_IELTS_BEFORE}'dan ${DEMO_IELTS_AFTER}'a çıkınca Uygun 2'den 5'e çıkar, Zorlayıcı 15'ten 12'ye iner`, () => {
    const before = bandCounts(DEMO_IELTS_BEFORE);
    const after = bandCounts(DEMO_IELTS_AFTER);

    expect(after.match).toBe(5);
    expect(after.reach).toBe(12);

    // Asıl iddia bu: katalog büyüse de fark ±3 kalıyor.
